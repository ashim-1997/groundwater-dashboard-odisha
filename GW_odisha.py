...
# Select all checkbox (same style as wells)
select_all_seasons = st.sidebar.checkbox("Select All Seasons", value=True)
...
select_all_wells = st.sidebar.checkbox("Select All Wells", value=False)
...
if select_all_wells:
    ...
    selected_wells = st.sidebar.multiselect(
        "Select Wells",
        well_options,
        default=[]   # 🔥 No wells selected initially
    )
...
if menu == "Well Trends":
    ...
    if filtered_df.empty:
     ...
     # Ensure numeric groundwater values
        ...
        # 🔥 Create combined label (Well + Season)
        well_trend_df["Well_Season"] = (
            well_trend_df["Well_ID"].astype(str) + " | " + well_trend_df["Season"]
        )
        ...
        fig_well_trend = px.line(
            ...
            x="Year",
            y="GWL",
            color="Well_Season",   # 👈 KEY CHANGE
            markers=True,
            title="Groundwater Level Trend by Well & Season"
        )

        fig_well_trend.update_layout(
            xaxis_title="Year",
            yaxis_title="Groundwater Level (m bgl)",
            legend_title="Well | Season",
            height=500
        )
...
if menu == "Monitoring Network":

    st.subheader("🗺️ Groundwater Monitoring Network")

    if df_wells.empty:
        st.warning("No well network data available.")
    else:

        # Ensure numeric coordinates
        df_wells["Latitude"] = pd.to_numeric(df_wells["Latitude"], errors="coerce")
        df_wells["Longitude"] = pd.to_numeric(df_wells["Longitude"], errors="coerce")

        df_wells = df_wells.dropna(subset=["Latitude", "Longitude"])

        # ---------------- Zone Filter ----------------
        zone_filter = st.multiselect(
            "Select Zone",
            df_wells["zone"].unique(),
            default=df_wells["zone"].unique()
        )

        filtered_wells = df_wells[df_wells["zone"].isin(zone_filter)].copy()
        ...
        def get_color(zone):
            if zone == "Lineament":
                ...
                return [0, 0, 255]       # Blue
            elif zone == "Lineament-River":
                return [255, 165, 0]     # Orange
            else:
                return [0, 255, 0]       # Green (Grid)

        filtered_wells["color"] = filtered_wells["zone"].apply(get_color)
        ...
        latitude=filtered_wells["Latitude"].mean(),
        longitude=filtered_wells["Longitude"].mean(),
        zoom=10
        )
